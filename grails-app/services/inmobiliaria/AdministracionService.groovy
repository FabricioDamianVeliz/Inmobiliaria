...
class AdministracionService {
      ...
      List listarVentasbuscador(String rubro, String localidad, String barrio) {
            def listadoventasbuscador = Propiedad.findAll("from Propiedad as p where (p.rubro=:rubros and p.localidad=:localidades and p.barrio=:barrios and p.estado=:libre)", [rubros: rubro, localidades: localidad, barrios: barrio, libre:'Disponible'])
      ...
      void quitarPropiedad(Long id){
            ...
            List listarrealizado() {
                  def realizado = Propiedad.findAll("from Propiedad as p where p.estado=:estado", [estado: 'No Disponible'])
                  return realizado
              }


          List listartodasconsultas() {
              def listado = Consulta.findAll("from Consulta as c where c.estado=:estado1 or c.estado=:estado2", [estado1: 'Leido', estado2: 'No Leido'])
                return listado
                    }
